...
func NewResizer(ctx context.Context, kubeClient *clientset.Clientset,
	configFile, namespace, helperImage, configMapName, serviceAccountName, helperPodYaml string, allowUnsafeHelperPodTemplate bool, name string, provisionerName string) (*LocalPathResizer, error) {
	var err error
	csiController, err := NewCsiController(ctx, kubeClient, configFile, namespace, helperImage, configMapName, serviceAccountName, helperPodYaml, allowUnsafeHelperPodTemplate)
	if err != nil {
		return nil, err
	}
	r := &LocalPathResizer{
